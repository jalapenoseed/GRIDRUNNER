...
import {sensorPaletteControls,THERMAL_PALETTES} from './dist/sensors.js';
import * as storyDiscoveries from './dist/story-discoveries.js';
import * as storyCampaign from './dist/story-campaign.js';
import {adminFleetAccess} from './dist/admin-fleet-ui.js';
...
import {renderSwarmCommand} from './dist/swarm-command.js';
...
const ctx=vm.createContext({...tutorialLibraryModule,sensorPaletteControls,THERMAL_PALETTES,swarmNotice:"",...sensorPackages,...fieldFlow,...onboarding,...fieldUpgrade,...squadron,...fieldBook,...backpack,...campaignProgress,...fleetPolicy,...fleetPolicyUI,backpackOptions:()=>({state:ctx.s,started:true,capacity:40,nearStorage:()=>false,nearby:null,recipes:[],canCraft:()=>false}),manualSection:"start",rigFocus:"reserve",narrator:{stop(){},read(){}},renderMaraDialogue:()=>"<p>Mara</p>",remap:{},finishInterface(){},introControls:()=>({mount:'F',use:'E',drone:'Q',lift:'SPACE / SHIFT'}),...intro,fleetCards,flightSession:null,campaignReturn:null,augmentFlightPanel(){},...relay,...relayWorldModule,...relayUI,makeRelayHouseWorld:(scene,solids)=>relayWorldModule.makeRelayHouseWorld(scene,solids,{assets:false}),machineSettings,drawInstruments,ControllerBridge,...experience,CameraManager,augmentPOVPanel(){},...settlements,...survival,augmentFieldPanel(){},augmentProgressionPanel(){},fieldJob:null,activeField:null,T,createDrone,DRONE_CLASSES,PRESETS,mobile:false,augmentV7Panel(){},renderSwarmCommand,mountSwarmCommand(){},unmountSwarmCommand(){},unmountSwarmProgram(){},releaseDroneView(){},leg3Defaults,leg3Sites,objective3,antennaCorrect,endingCost,sound:{event(){},setLevels(){},start(){},pause(){}},leg2Defaults,leg2Sites,leg2Objective,phaseInput,hydroAvailable,SAVE_VERSION,validateSave,generationStep,console,Date,Math,JSON,Number,Blob,setTimeout,URL,performance,localStorage:{getItem:k=>memory.get(k)||null,setItem:(k,v)=>memory.set(k,v)},$ :node,document:{body:{style:{setProperty(){}},classList:{toggle(){}}}},clamp:(v,a,b)=>Math.max(a,Math.min(b,v)),settings:null,bike:{position:new T.Vector3(1,0,3),rotation:{}},trailer:{position:new T.Vector3(2,0,5),rotation:{}},droneOrigin:null,crates:Array.from({length:4},()=>({done:false,mesh:{visible:true}})),enemies:[[15,-575],[138,-704]].map(home=>({home,x:home[0],z:home[1],hp:75,mesh:{position:new T.Vector3(),rotation:new T.Euler()}})),hud(){},toast(){},radio(){},play(){},screen:'start',nearest:null,keys:{},drawMap(){},cargo:()=>0,cargoLimit:()=>40,recipes:[],can:()=>false,dist:()=>20,weights:{},scene:{},renderer:{},camera:{},time:0});
ctx.packsContext=()=>({enabled:true,near:true,stopped:true});
Object.assign(ctx,storyDiscoveries,storyCampaign,{adminFleetAccess},swarmOps,fleetManifest,surveillance,openingRoute,{queueSwarmAircraft(){},operatorConcealed:()=>false,renderLiveSwarm(){node('#panel').innerHTML=swarmOps.renderSwarmOps({state:ctx.s,started:true});}});
Object.assign(ctx,batteryPacks,batteryPackUI,fleetTasks,fleetTaskUI,powerLines,lineHarvest,lineHarvestUI);
vm.runInContext(src.slice(src.indexOf('function fleetAvailable('),src.indexOf('function setFormation(')),ctx);
