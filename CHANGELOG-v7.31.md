# v7.31 — Personal thermal and neon escort

Based on Grok's `812380d` v7.29 update, merged with its concurrent `90fa399` v7.30 contact-AO update. Existing presence, foliage, animation, audio, fleet jobs and campaign features are retained.
...
- **Presence repair:** HDR render targets, final exposure/tone mapping/display conversion and drawing-buffer sizing correct the v7.29 bloom output. Vignette uses a defined smoothstep direction. Thermal remains on its separate renderer path. The shared blur kernel now preserves constant brightness, which is required by the newly merged contact-AO pass.
...
- Constant-color render matched `[84,84,84,255]` both with and without postprocessing. Normal forward camera projection explicitly checks that the close ring is in the viewport.
