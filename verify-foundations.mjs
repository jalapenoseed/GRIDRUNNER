...
// A lab reference must never become a model input or a fabricated prediction.
let sentFrame;const labDetector=new VisionDetector({document:doc,workerFactory:()=>({postMessage:m=>{if(m.kind==='frame')sentFrame=m;},terminate(){}})});
labDetector.canvas.getContext=()=>({fillRect(){},drawImage(){},getImageData:()=>({data:new Uint8ClampedArray(416*416*4)})});
labDetector.toggle();labDetector.message({kind:'ready'});labDetector.update({width:800,height:400},performance.now(),{reference:[{label:'person',x1:80,y1:40,x2:160,y2:200}],pose:[0,0,0]});
assert(sentFrame.pixels instanceof ArrayBuffer);assert(!('reference' in sentFrame));assert.equal(labDetector.pending.reference.length,1);labDetector.message({kind:'result',id:labDetector.pending.id,boxes:[],ms:5});labDetector.update(null,performance.now(),{pose:[0,0,0]});assert(labDetector.readout.textContent.includes('MISSED 1'));assert.equal(labDetector.layer.childElementCount,0);labDetector.toggle();
