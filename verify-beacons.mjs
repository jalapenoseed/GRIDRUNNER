...
for(const [id,r]of Object.entries(fleet.records)){assert(r.root.visible);const b=r.root.userData.beacon;assert(b.visible);assert.equal(b.material.color.getHex(),BEACONS[id].color);assert(b.material.depthTest&&!b.material.depthWrite&&!b.material.fog&&!b.material.toneMapped);assert(b.material.opacity>.9);}
...
console.log('PASS: four permanent beacon identities, compact 10px projection at 150/460/900m across viewport heights, occlusion flags, thermal deployed/perched/relay heat and material restoration. GPU appearance remains a playtest.');
