...
const renderer={render(){for(const r of Object.values(fleet.records)){r.proxy.traverse(o=>{if(o.isMesh)assert(o.material.userData.sensorHeat>=.7&&o.material.depthTest&&!o.material.fog);});assert.equal(r.root.userData.beacon.material.color.getHex(),BEACONS[r.id].color);}assert(wall.material.userData.sensorHeat<.5);assert(sensors.hot.depthTest&&!sensors.hot.fog);}};
