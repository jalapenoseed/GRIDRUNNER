...
fleet.update(.02,state,new T.Object3D(),new T.Object3D(),camera);const beacon=fleet.records.scout.root.userData.beacon;assert.equal(beacon.material.blending,T.NormalBlending);assert(beacon.material.opacity>=.88);assert(beacon.scale.x<4,'Compact point at 300 m');assert(beacon.material.depthTest,'World geometry still occludes beacons');
