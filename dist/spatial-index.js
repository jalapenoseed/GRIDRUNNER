// Broad-phase index for GRIDRUNNER's static AABB collision volumes. The
...
export class SpatialIndex{
 ...
 key(x,z){return x+','+z;}
 rebuild(){this.cells.clear();for(const body of this.source){const x0=Math.floor((body.x-body.w)/this.cellSize),x1=Math.floor((body.x+body.w)/this.cellSize),z0=Math.floor((body.z-body.d)/this.cellSize),z1=Math.floor((body.z+body.d)/this.cellSize);for(let x=x0;x<=x1;x++)for(let z=z0;z<=z1;z++){const key=this.key(x,z),bucket=this.cells.get(key);if(bucket)bucket.push(body);else this.cells.set(key,[body]);}}this.count=this.source.length;return this;}
 ensure(){if(this.count!==this.source.length)this.rebuild();}
 all(){this.ensure();return this.source;}
 queryAABB(minX,minZ,maxX,maxZ){this.ensure();const found=new Set(),x0=Math.floor(minX/this.cellSize),x1=Math.floor(maxX/this.cellSize),z0=Math.floor(minZ/this.cellSize),z1=Math.floor(maxZ/this.cellSize);for(let x=x0;x<=x1;x++)for(let z=z0;z<=z1;z++)for(const body of this.cells.get(this.key(x,z))||[])found.add(body);return [...found];}
