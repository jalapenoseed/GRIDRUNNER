...
export class MenuMemory{
 ...
 restore(panel,screen,group){this.current=screen;if(group)this.children[group.id]=screen;const saved=this.pages[screen],content=panel.querySelector('#fieldContent');if(!content||!saved)return;
  ...
  const target=[...content.querySelectorAll('button,input,select,summary')].find(el=>signature(el)===saved.focus&&!el.disabled);target?.focus({preventScroll:true});content.scrollTop=Math.max(0,Math.min(Number(saved.scroll)||0,100000));
 }
