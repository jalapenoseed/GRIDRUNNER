...
export function renderSwarmProgram(program=createSwarmProgram(),started=false){
 const fleetIds=program.fleetIds||STARTER_AIRCRAFT,p=program.settings,number=(key,label,step=1)=>`<label>${label}<input data-program-number="${key}" type="number" step="${step}" min="${PROGRAM_RANGES[key][0]}" max="${PROGRAM_RANGES[key][1]}" value="${p[key]}" required></label>`,select=(key,label,values)=>`<label>${label}<select data-program-setting="${key}">${choices(values,p[key])}</select></label>`;
 ...
 <fieldset class="programFleet"><legend>Affected aircraft</legend>${fleetIds.map(id=>`<label><input type="checkbox" data-program-id="${id}" ${program.ids.includes(id)?'checked':''}>${aircraftCode(id)}</label>`).join('')}<button data-program-action="all">ALL AIRCRAFT</button><button data-program-action="scout-pair">SCOUTS 03 + 04</button></fieldset>
...
export function mountSwarmProgram(root,{program,started,reducedMotion=false,onDraft=()=>{},onApply=()=>'',onResume=()=>{},onLivePause=()=>'',onReturn=()=>''}){
 ...
 function validate(){try{valid=validateSwarmProgram(draft,{reset:true,aircraft:draft.fleetIds||STARTER_AIRCRAFT});error.textContent='';if(draft.mode==='script'){const script=compileProgram(draft.source,{aircraft:draft.fleetIds||STARTER_AIRCRAFT});find('[data-program-cues]').innerHTML=script.cues.filter(c=>c.assignment||Object.keys(c.patch).length).map(c=>`<li><b>${c.at}s · line ${c.line}</b> ${esc(c.assignment?'assign '+c.assignment:Object.entries(c.patch).map(([k,v])=>k+' '+v).join(' · '))}</li>`).join('');}return true;}catch(e){error.textContent=e.message;valid=null;running=false;return false;}}
 ...
 listen(root,'change',async event=>{
  const el=event.target;
  if(el.dataset.programId){draft.ids=(draft.fleetIds||STARTER_AIRCRAFT).filter(id=>find('[data-program-id="'+id+'"]').checked);changed();}
  ...
  if(el.hasAttribute('data-program-import'))try{const file=el.files?.[0];if(!file)return;if(file.size>32000)throw Error('Preset must be smaller than 32 KB.');const text=await file.text();if(!alive)return;const imported=validateSwarmProgram(JSON.parse(text),{reset:true,aircraft:draft.fleetIds||STARTER_AIRCRAFT});draft=imported;t=0;running=false;updateControls();message.textContent='Preset imported. Preview it, then Apply to fly.';}catch(e){if(alive)error.textContent=e.message;}
 });
 ...
 listen(root,'click',event=>{
  ...
  if(action==='all'||action==='scout-pair'){draft.ids=action==='all'?[...(draft.fleetIds||STARTER_AIRCRAFT)]:['scout-03','scout-04'].filter(id=>(draft.fleetIds||STARTER_AIRCRAFT).includes(id));updateControls();}
  if(action==='example'){draft.source=PROGRAM_EXAMPLES[find('[data-program-example]').value];draft.ids=[...(draft.fleetIds||STARTER_AIRCRAFT)];draft.mode='script';t=0;running=false;updateControls();message.textContent='Example loaded for the fleet. Edit the affected group to keep guards on duty.';}
  ...
  if(action==='load')try{const value=presets()[find('[data-program-saved]').value];if(!value)throw Error('Choose a saved preset.');draft=validateSwarmProgram(value,{reset:true,aircraft:draft.fleetIds||STARTER_AIRCRAFT});t=0;running=false;updateControls();message.textContent='Preset loaded. Apply when ready.';}catch(e){error.textContent=e.message;}
