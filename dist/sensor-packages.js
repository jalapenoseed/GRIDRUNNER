...
export function sensorPanel(type,active='visible'){return `<h3>Sensor payload</h3><p>B changes sensor · R measures · Y toggles independent pixel detection.</p><div class="commandGrid sensorButtons">${availableSensors(type).map(k=>`<button data-sensor-mode="${k}" aria-pressed="${active===k}">${SENSOR_SPECS[k].name}<small>${SENSOR_SPECS[k].cost} charge / scan</small></button>`).join('')}</div><p class="hint">UV reveals authored fluorescent paint at short range. Thermal and acoustic readings identify authored heat and sound sources. RF detects authored emitters. Try Flight Yard → Sensor / YOLO Lab to compare all three. Depth measures contact distance. These sensors do not reveal puzzle answers or infer hidden items from YOLO.</p>`;}
