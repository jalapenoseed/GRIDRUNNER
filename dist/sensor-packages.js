...
export const SENSOR_PACKAGES={scout:['visible','night','acoustic'],cargo:['visible','depth'],engineer:['visible','thermal','depth'],relay:['visible','rf']};
...
export function sensorReading(mode,entity,distance,radius,blocked=0){
 ...
 const reading=mode==='depth'?`${distance.toFixed(1)} m range`:mode==='thermal'?(entity.kind==='HOSTILE'?'Warm moving body':'Warm equipment surface'):mode==='rf'?`Emitter strength ${Math.round(100/(1+(distance/80)**2))}%`:mode==='acoustic'?'Rotor / movement sound':mode==='night'?'Low-light silhouette':'Optical classification';
 return {sensor:mode,source:'world-simulation',confidence,reading};
}
export function sensorPanel(type){return `<h3>Sensor payload</h3><p>B changes sensor · R measures · Y toggles independent pixel detection.</p><div class="chips">${availableSensors(type).map(k=>`<span>${SENSOR_SPECS[k].name} / ${SENSOR_SPECS[k].cost} charge per scan</span>`).join('')}</div><p class="hint">Thermal and acoustic readings identify authored heat and sound sources. RF detects authored emitters. Depth measures contact distance. These sensors do not reveal puzzle answers or infer hidden items from YOLO.</p>`;}
