...
export class Immersion{
 constructor(scene,renderer,camera,sun,bike,drone){
  ...
  this.sky=new T.Mesh(new T.SphereGeometry(2800,32,16),new T.ShaderMaterial({
   side:T.BackSide,depthWrite:false,fog:false,
   uniforms:{top:{value:new T.Color()},bottom:{value:new T.Color()},sunDirection:{value:new T.Vector3(-.5,.8,-.6)},sunStrength:{value:1},sunColor:{value:new T.Color(0xfff2dc)},cloudCover:{value:0},cloudLight:{value:1},time:{value:0}},
   vertexShader:'varying vec3 v; void main(){v=position; gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',
   fragmentShader:`uniform vec3 top;uniform vec3 bottom;uniform vec3 sunDirection;uniform vec3 sunColor;
    uniform float sunStrength;uniform float cloudCover;uniform float cloudLight;uniform float time;varying vec3 v;
    float hash(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453);}
    float noise(vec2 p){vec2 i=floor(p),f=fract(p);f=f*f*(3.-2.*f);return mix(mix(hash(i),hash(i+vec2(1.,0.)),f.x),mix(hash(i+vec2(0.,1.)),hash(i+vec2(1.,1.)),f.x),f.y);}
    void main(){
     vec3 direction=normalize(v);float h=max(direction.y,0.);vec3 c=mix(bottom,top,pow(h,.52));
     float alignment=max(0.,dot(direction,normalize(sunDirection)));
     c+=sunColor*(pow(alignment,1100.)*2.+pow(alignment,14.)*.12)*sunStrength;
     if(cloudCover>0.&&h>.025){
      vec2 p=direction.xz/(h+.22)*3.2+vec2(time*.003,0.);
      float cloud=smoothstep(.47,.78,noise(p)*.7+noise(p*2.3)*.3);
      cloud*=smoothstep(.025,.18,h)*cloudCover;
      c=mix(c,mix(bottom,vec3(.88,.91,.91),.5)*cloudLight,cloud);
     }
  ...
  const batch=(geometry,color,count,place,roughness=.94,respectLayout=true)=>{
   geometry.computeBoundingSphere();const radius=geometry.boundingSphere.radius;
   const m=new T.InstancedMesh(geometry,new T.MeshStandardMaterial({color,roughness}),count);let accepted=0;
   for(let i=0;i<count;i++)for(let attempt=0;attempt<32;attempt++){
    place(dummy,i);const footprint=radius*Math.max(dummy.scale.x,dummy.scale.z);
    if(respectLayout&&!sceneryAllowed(dummy.position.x,dummy.position.z,footprint))continue;
    dummy.updateMatrix();m.setMatrixAt(accepted++,dummy.matrix);break;
   }
   m.count=accepted;m.instanceMatrix.needsUpdate=true;m.castShadow=false;m.receiveShadow=true;m.userData.maxCount=accepted;m.userData.sceneryClearance=respectLayout;
   m.computeBoundingSphere();scene.add(m);this.instances.push(m);return m;
  };
