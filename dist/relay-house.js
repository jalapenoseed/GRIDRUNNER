import {createCircuits,validateCircuits,recordCircuitClue} from './relay-circuits.js';
...
export function createRelayHouse(){return {version:1,circuits:createCircuits(),...Object.fromEntries(FLAGS.map(k=>[k,false])),frequency:0,selected:'',evidence:[]};}
export function validateRelayHouse(raw){if(raw===undefined)return createRelayHouse();if(!raw||raw.version!==1)throw Error('Invalid relay house');const out=createRelayHouse();for(const k of FLAGS){if(typeof raw[k]!=='boolean')throw Error('Invalid relay flag');out[k]=raw[k];}if(![0,147.2].includes(raw.frequency)||typeof raw.selected!=='string'||raw.selected&&!Object.hasOwn(ITEMS,raw.selected)||!Array.isArray(raw.evidence)||raw.evidence.length>Object.keys(EVIDENCE).length||new Set(raw.evidence).size!==raw.evidence.length||raw.evidence.some(id=>!Object.hasOwn(EVIDENCE,id)))throw Error('Invalid relay evidence');out.frequency=raw.frequency;out.selected=raw.selected;out.evidence=[...raw.evidence];out.circuits=validateCircuits(raw.circuits,{legacySolved:raw.radioTuned});if(out.radioTuned&&!out.circuits.solved)throw Error('Relay bus not commissioned');if(out.power&&(!out.generatorFixed||!out.fuseSeated)||out.radioTuned&&(!out.power||!out.dishAligned||!out.filterInstalled||out.frequency!==147.2)||out.discovered&&!out.radioTuned||out.loggedIn&&(!out.noteRead||!out.frequencyRead))throw Error('Inconsistent relay progression');return out;}
export function recordEvidence(r,id){if(!r.evidence.includes(id))r.evidence.push(id);}
export function relayObjective(r){if(r.discovered)return 'Carrier decoded. The token opens the northern tower.';if(!r.fuseTaken)return 'Explore the cellar. Follow the stairs down beside the bunk room.';if(!r.fuseSeated)return 'Bring the protected fuse to Len in the kitchen.';if(!r.generatorFixed)return 'Free the yard generator with a wrench.';if(!r.frequencyRead||!r.noteRead)return 'Join Len’s note with the carrier scrap in the office.';if(!r.loggedIn)return 'Log in at the office CRT using the two clues.';if(!r.filterInstalled)return 'Read the notebook, wind a coil, build and install a signal filter.';if(!r.dishAligned)return 'Fly to the roof and release the dish service latch.';if(!r.circuits.solved)return 'Read the electrical measurements, then commission the distribution board at the office CRT.';if(r.frequency!==147.2)return 'Use the terminal to tune the carrier from the scrap.';return 'Listen at the cellar radio rack.';}
export function useRelay(r,id,{mode='foot',inv={},capacity=24,selected=r.selected}={}){
 ...
 if(id==='generator'){recordCircuitClue(r.circuits,'bus');recordEvidence(r,'service');if(r.power)return fail('Generator running. The cabin circuit is stable.');if(!r.generatorFixed){if(!have('wrench')||!chosen('wrench'))return fail('The governor is seized. Select a wrench in Pack, then use the generator.');r.generatorFixed=true;}r.power=r.generatorFixed&&r.fuseSeated;return ok(r.power?'Generator running. Kitchen lights and terminal are online.':'Governor freed. Fit the kitchen fuse to close the circuit.','generatorCrank');}
 if(id==='toolbox'){recordCircuitClue(r.circuits,'fault');if(r.fuseTaken)return fail('Toolbox empty. The maintenance label stays in your journal.');const error=transform(inv,{}, {fuse:1,copper:2,plastic:1,wire:1,electronics:1},capacity);if(error)return fail(error);r.fuseTaken=true;recordEvidence(r,'service');return ok('Protected fuse, copper, insulation, wire and a board recovered. Len needs the fuse.');}
 ...
 if(id==='bunk'){recordCircuitClue(r.circuits,'loads');r.schematicRead=true;recordEvidence(r,'schematic');return ok('Signal filter and reserve-cell schematics learned. See Pack / Fieldwork.');}
 ...
 if(id==='dish'){if(mode!=='drone')return fail('The roof service latch requires your FPV drone.');if(!r.power)return fail('Dish actuator has no power. Restore the house first.');recordCircuitClue(r.circuits,'roof');if(r.dishAligned)return fail('Dish already points north.');r.dishAligned=true;return ok('Dish aligned. Branch B service rating recorded in Journal. Commission the office distribution board.','carrierLock');}
 if(id==='radio'){
  ...
  if(!r.circuits.solved)return fail('Distribution bus not commissioned. Compare the recorded measurements and test the office circuit board.');
...
export function terminalCommand(r,command){
 ...
 if(verb==='read'){if(args[0]==='filter.sch'){recordCircuitClue(r.circuits,'loads');r.schematicRead=true;recordEvidence(r,'schematic');return EVIDENCE.schematic.text;}recordEvidence(r,'log');return EVIDENCE.log.text;}
