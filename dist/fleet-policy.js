...
export function createFleetPolicy(){return {version:1,enabled:false,targetPercent:60,cycles:0,lastCompletedTaskId:'',acknowledgedTaskId:'',retryAt:0,status:'Complete one manual delivery to unlock reserve automation.'};}
...
export function validateFleetPolicy(value){
 ...
 if(value.acknowledgedTaskId!==undefined&&(typeof value.acknowledgedTaskId!=='string'||value.acknowledgedTaskId.length>80))throw Error('Invalid policy acknowledgement');
 return {...p,...value};
}

export function suspendFleetPolicy(s,reason){if(!s.fleetPolicy?.enabled)return;s.fleetPolicy.enabled=false;s.fleetPolicy.status='Suspended / '+reason+'; re-arm to continue.';}
...
export function configureFleetPolicy(s,{enabled,targetPercent}={}){
 ...
 if(enabled!==undefined){if(enabled&&!campaignUnlocks(s).reservePolicy)return {ok:false,reason:'Complete one manual Harvest & Deliver run first.'};p.enabled=!!enabled;p.acknowledgedTaskId=['FAILED','CANCELLED'].includes(s.squad?.engineer?.task?.state)?s.squad.engineer.task.id:'';p.retryAt=0;p.status=p.enabled?'Reserve automation armed.':'Reserve automation off; active flights continue safely.';}
...
export function advanceFleetPolicy(s,record,{elapsed=0,nearTrailer=false,trailerStopped=false,trailerHome,home,solids=[],terrain=()=>0,storm=false,jammed=false}={}){
 ...
 if(task?.kind==='LINE'&&task.packId&&task.state==='COMPLETED'&&task.stage==='DONE'&&task.id!==p.lastCompletedTaskId){
  const first=p.cycles===0;p.lastCompletedTaskId=task.id;p.cycles++;syncCampaignProgress(s).firstPackDelivered=true;if(first)events.push('policy-unlocked');
 }
 ...
 if(p.enabled&&['CANCELLED','FAILED'].includes(task?.state)&&task.id!==p.acknowledgedTaskId)suspendFleetPolicy(s,'job '+task.state.toLowerCase());
 if(!p.enabled){if(!p.status.startsWith('Suspended'))p.status='Unlocked / manual control.';return events;}
