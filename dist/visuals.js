import {detailBike,detailTrailer,detailPerson} from './field-fabrication.js';
...
export function combineStatic(group){group.updateMatrixWorld(true);const groups=new Map();for(const m of [...group.children]){if(!m.isMesh)continue;m.updateMatrix();const g=m.geometry.index?m.geometry.toNonIndexed():m.geometry.clone();g.applyMatrix4(m.matrix);if(!groups.has(m.material))groups.set(m.material,[]);groups.get(m.material).push(g);group.remove(m);m.geometry.dispose();}for(const [mat,parts]of groups){const n=parts.reduce((n,p)=>n+p.attributes.position.array.length,0),pos=new Float32Array(n),norm=new Float32Array(n),uv=new Float32Array(n/3*2);let o=0,u=0;for(const p of parts){pos.set(p.attributes.position.array,o);norm.set(p.attributes.normal.array,o);if(p.attributes.uv)uv.set(p.attributes.uv.array,u);u+=p.attributes.position.count*2;o+=p.attributes.position.array.length;p.dispose();}const g=new T.BufferGeometry();g.setAttribute('position',new T.BufferAttribute(pos,3));g.setAttribute('normal',new T.BufferAttribute(norm,3));g.setAttribute('uv',new T.BufferAttribute(uv,2));group.add(new T.Mesh(g,mat));}return group;}
export function makeBike(){const g=new T.Group();wheel(g,0,.58,-1);wheel(g,0,.58,1);for(const x of [-.18,.18]){rod(g,[x,.58,1],[x,1.23,.1],.045,palette.steel);rod(g,[x,1.23,.1],[x,.65,-.6],.045,palette.black);rod(g,[x,.65,-.6],[x,.7,.7],.055,palette.black);rod(g,[x,.58,-1],[x,1.5,-.6],.045,palette.steel);rod(g,[x,1.35,.3],[x,.68,.9],.04,palette.amber);}box(g,0,1.02,0,.42,.65,.64,palette.black);box(g,0,1.44,.35,.46,.14,.9,palette.rubber);box(g,0,.72,-.04,.46,.12,.8,palette.edge);rod(g,[-.65,1.65,-.65],[.65,1.65,-.65],.045,palette.black);box(g,0,1.42,-.8,.4,.28,.16,palette.edge);mesh(g,new T.CircleGeometry(.11,12),0,1.42,-.891,0xe9f5da,true).rotation.y=Math.PI;crate(g,-.48,1.15,.75,.4,.48,.55);crate(g,.48,1.15,.75,.4,.48,.55);box(g,0,1.1,1.3,.25,.08,.045,0xc44933);detailBike(g);return combineStatic(g);}
...
export function makeWeapon(){const g=new T.Group();box(g,.28,-.3,-.67,.1,.13,.4,palette.black);box(g,.28,-.23,-.69,.045,.02,.32,palette.steel);box(g,.28,-.39,-.53,.07,.19,.08,palette.rubber);rod(g,[.28,-.28,-.85],[.28,-.28,-1.14],.025,palette.edge);box(g,.32,-.285,-.72,.015,.035,.1,palette.cyan);return combineStatic(g);}
export function makePerson(x,z,c){
 ...
 for(const side of [-1,1]){box(g,side*.18,1.42,.265,.13,.18,.06,0x727b66);box(g,side*.18,1.24,.255,.15,.14,.06,0x494c3e);box(g,side*.15,1.69,.15,.035,.2,.04,0xb2a88b);
  ...
  box(pivot,0,-.76,.09,.25,.19,.43,0x17201f);box(pivot,0,-.42,.12,.19,.21,.08,0x555b4b);combineStatic(pivot);g.add(pivot);legs.push(pivot);
  const arm=new T.Group();arm.position.set(side*.33,1.66,0);rod(arm,[0,0,0],[side*.08,-.35,.04],.09,c);rod(arm,[side*.08,-.35,.04],[side*.08,-.62,.19],.08,c);mesh(arm,new T.SphereGeometry(.088,8,6),side*.08,-.67,.2,0x282e29);combineStatic(arm);g.add(arm);arms.push(arm);
 }
 ...
 combineStatic(head);g.add(head);box(g,0,1.44,-.24,.45,.58,.21,0x303b37);box(g,0,1.55,-.36,.28,.13,.03,0xa79668);detailPerson(g,{head,arms,legs,color:c});for(const part of [head,...arms,...legs])combineStatic(part);g.userData.head=head;g.userData.arms=arms;g.userData.legs=legs;g.position.set(x,0,z);return attachFieldRig(combineStatic(g));
}
...
export function makeMaraShelter(solids=[]){
 ...
 g.userData.mara={x:54,z:-94};return combineStatic(g);
}
...
for(const [x,z]of [[-85,-290],[90,-416],[155,-733]]){rod(g,[x+4,0,z+4],[x+4,3.7,z+4],.055,palette.steel);box(g,x+4,3.4,z+4,1.8,.6,.12,0x25383a);mesh(g,new T.SphereGeometry(.14,8,6),x+4,4,z+4,palette.cyan,true);}return combineStatic(g);}
