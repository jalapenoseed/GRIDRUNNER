...
vec2 uv=vUv*2.-1.;float v=smoothstep(1.15,.35,dot(uv,uv));c*=mix(1.,v,vignette);
...
export class PresenceComposer{
  ...
  _boot(w,h){
    ...
    const opts={minFilter:T.LinearFilter,magFilter:T.LinearFilter,depthBuffer:true};
  ...
  render(){
    ...
    const size=r.getSize?.(new T.Vector2())||{x:innerWidth,y:innerHeight};
