...
export class EnvironmentDetail{
 constructor(scene,{textures=true}={}){
  ...
  for(let leg=1;leg<=3;leg++){
   ...
   batch(new T.IcosahedronGeometry(1,1),this.materials.rock,130,leg,(o)=>{const p=placeScenery(()=>({x:(random()<.5?-1:1)*(16+random()*9),z:start-random()*1400}),1.4);if(!p)return false;const {x,z}=p;o.position.set(x,heightAt(x,z)+.18,z);o.scale.set(.25+random()*.9,.16+random()*.35,.3+random());o.rotation.set(random(),random()*6,random());});
