...
export class EnvironmentDetail{
 constructor(scene,{textures=true,solids=[]}={}){
  ...
  for(let leg=1;leg<=3;leg++){
   ...
   batch(new T.IcosahedronGeometry(1,1),this.materials.rock,130,leg,(o)=>{const p=placeScenery(()=>({x:(random()<.5?-1:1)*(16+random()*9),z:start-random()*1400}),1.4);if(!p)return false;const {x,z}=p,sx=.25+random()*.9,sy=.16+random()*.35,sz=.3+random();o.position.set(x,heightAt(x,z)+.18,z);o.scale.set(sx,sy,sz);o.rotation.set(random(),random()*6,random());if(Math.max(sx,sz)>.72)solids.push({x,z,w:sx*.72,d:sz*.72,minY:heightAt(x,z),maxY:heightAt(x,z)+Math.max(.45,sy*1.7),kind:'rock'});});
