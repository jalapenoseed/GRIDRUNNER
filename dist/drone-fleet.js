import {storyOwns,storyStation} from './story-campaign.js';
...
function neonAirframe(root,id,span){
 ...
 const halo=new T.Sprite(new T.SpriteMaterial({map:beaconMap,color,blending:T.AdditiveBlending,transparent:true,opacity:.12,depthWrite:false,toneMapped:false,fog:false}));
 halo.position.y=.4;rig.add(halo);
 const material=new T.MeshBasicMaterial({color:new T.Color(color).multiplyScalar(.65),toneMapped:false,fog:false});
 for(const side of [-1,1]){
  ...
  for(const end of [-1,1]){
   ...
   lamp.position.set(side*span*.35,.025,end*span*.32);lamp.scale.setScalar(.1);lamp.material.opacity=.4;rig.add(lamp);
  }
 }
 const ring=new T.Mesh(new T.TorusGeometry(span*.2,.02,5,24),material);ring.rotation.x=Math.PI/2;ring.position.y=-.16;ring.visible=false;rig.add(ring);
...
export class DroneFleet{
 ...
 update(dt,s,trailer,bike,camera,{reducedMotion=false}={}){const squad=syncSquad(s);const active=this.records[s.droneType]||this.records.scout;if(this.shouldStream(active.id,s))this.load(active.id);
  for(const r of Object.values(this.records)){if(!storyOwns(s,r.id)||storyStation(s,r.id)?.leg&&storyStation(s,r.id).leg!==s.leg){r.root.visible=false;continue;}const selected=r===active,d=squad[r.id].system,flying=d.mode!=='DOCK';const onboard=selected&&s.mode==='drone'&&!['chase','overhead'].includes(s.experience?.preferred?.drone);r.root.visible=flying?!onboard:STARTER_AIRCRAFT.includes(r.id)||selected&&s.mode!=='drone';if(flying&&this.shouldStream(r.id,s))this.load(r.id);const beacon=r.root.userData.beacon;beacon.visible=flying;r.root.userData.lineClamp.visible=!!d.perch||storyStation(s,r.id)?.kind==='line';beacon.material.color.setHex(BEACONS[r.id].color);beacon.material.opacity=.94+.06*Math.pow(Math.sin((s.elapsed||0)*4+FLEET.findIndex(f=>f.id===r.id)),8);const size=beaconScale(camera,new T.Vector3(...d.pos),globalThis.innerHeight||800);beacon.scale.set(size,size,1);const neon=r.root.userData.neon,halo=r.root.userData.beaconHalo;neon.visible=flying;halo.scale.set(size*1.5,size*1.5,1);halo.material.opacity=.12;const core=r.root.userData.beaconCore;core.visible=flying;core.scale.set(size*.32,size*.32,1);r.root.userData.batteryPack.visible=!!carriedPack(s,squad[r.id]);
